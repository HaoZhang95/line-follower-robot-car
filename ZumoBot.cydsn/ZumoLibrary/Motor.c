...
* @details  moving left when l_speed < r_speed or moving right when l_speed > r_speed
* @param    uint8 l_speed : left motor speed value
* @param    uint8 l_backward : 1 if backward, 0 if forward, left motor
* @param    uint8 r_speed : right motor speed value
* @param    uint8 r_backward : 1 if backward, 0 if forward, right motor
* @param    uint32 delay : delay time
*/
void motor_turn_adv(uint8 l_speed, uint8 l_backward, 
            uint8 r_speed, uint8 r_backward, uint32 delay)
{
    MotorDirLeft_Write(l_backward);      // set LeftMotor backward mode if 1
    MotorDirRight_Write(r_backward);     // set RightMotor backward mode if 1
    PWM_WriteCompare1(l_speed); 
    PWM_WriteCompare2(r_speed); 
    CyDelay(delay);
}
