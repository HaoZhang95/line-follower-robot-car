...
int main()
{
    ...
    struct sensors_ ref;
    struct sensors_ dig;  
    uint8_t detect = 0, countdown = 100, checkStop = 0, lastBlack = 0;
    ...
    // Robot start running and stop at the first black line
    uint8_t initialRun = 0;    
    motor_forward(100,0);
    while (initialRun != 2) {
        reflectance_read(&ref);
        printf("%d %d %d %d \r\n", ref.l3, ref.l1, ref.r1, ref.r3);
        // If it reached the first black line it will count to 1
        // if the robot is in our hand then do nothing.
        // (if 1111 then it is in our hand)
        // If it is put on the track then initiate state
        // if 0110
        if (ref.r1>SENSOR_TH&&ref.l1>SENSOR_TH&&ref.r3<SENSOR_TH&&ref.l3<SENSOR_TH) {
            initialRun = 1;
        }
        
        // if the robot not in our hand (inititalRun = 1)
        // Then it still move forward, so if it go out of the black line it will stop
        if (initialRun ==1 && ref.r3>=SENSOR_TH&&ref.l3>=SENSOR_TH) {
            initialRun = 2;
        }
    }
    ...
    motor_start();
    //Sumo mode move forward a little bit
    //Pathfinding should also move forward to avoid read the first black line
    ...
    {   
        ...
        //Sumoing(ref.r1, ref.r3, ref.l1, ref.l3);        
        PathfindingAdv(ref.r1, ref.r3, ref.l1, ref.l3, &checkStop, &lastBlack);
        //CyDelay(100);
    }
}   

// alert the users by make a non stop beep sound and stop motor
// if the battery level is low
void batteryAlert() {
    // function to alert here
    ADC_Battery_Start();        
    int16 adcresult =0;
    float volts = 0.0;
